...
import { createFractalShader, FractalShaderDescrition } from "./createFractalShader";

const convertCustomVarsToTypes = (customVars: Record<string, unknown>) => {
  return Object.entries(customVars).reduce((acc, [key, val]) => {
...
export class FractalImage {
  ...
  updateParams(newParams: FractalParamsBuildRules) {
    if (Object.keys(newParams.custom).length !== Object.keys(this.params.custom).length) {
      this.shader = createFractalShader(
        this.context,
        this.formula,
