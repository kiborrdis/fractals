...
void main() {
  ...
  float superSampling = 1.0; // n x n grid, should be even

  if (superSampling == 1.0) {
    iterationSmooth = generateFractalIntensity(gl_FragCoord.xy);
  } else {
    float sidePoints = superSampling / 2.0;

    float step = 1.0 / (superSampling + 1.0);
    ...
    for (float i = 0.0; i <= 16.0; i += 1.0) {
      ...
      for (float j = 0.0; j <= 16.0; j += 1.0) {
        ...
        vec2 samplePoint = gl_FragCoord.xy + vec2((i - sidePoints) * step, (j - sidePoints) * step);
