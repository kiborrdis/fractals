...
export const createFractalVisualizer = (
  ...
  const positionAttributeLocation = context.getAttribLocation(shaderProgram, "a_position");
  const resolutionUniformLocation = context.getUniformLocation(shaderProgram, "u_resolution");

  const colorStartUniformLocation = context.getUniformLocation(shaderProgram, "u_color_start");
  const colorEndUniformLocation = context.getUniformLocation(shaderProgram, "u_color_end");
  const colorOverflowUniformLocation = context.getUniformLocation(shaderProgram, "u_color_overflow");
  
  const fractalCLocation = context.getUniformLocation(shaderProgram, "u_fractal_c");
  const fractalRLocation = context.getUniformLocation(shaderProgram, "u_fractal_r");

  const fractalRRangeStartLocation = context.getUniformLocation(shaderProgram, "u_fractal_r_range_start");
  const fractalRRangeEndLocation = context.getUniformLocation(shaderProgram, "u_fractal_r_range_end");
  ...
  const colorStart = [0.1, 0.1, 0.1];
  const colorEnd = [1, 0.7, 0];
  const colorOverflow = [1, 0.5, 0];
  const fractalC = [-0.55, -0.595];
  const fractalR = 2;

  const fractalRRangeStart = [-1, -1];
  const fractalRRangeEnd = [1, 1];
  ...
  const loop = createRenderLoop({
    loopIterationCallback: async ({ timeDelta, timeSinceStart }) => {
      ...
      context.uniform3f(colorStartUniformLocation, colorStart[0], colorStart[1], colorStart[2]);
      context.uniform3f(colorEndUniformLocation, colorEnd[0], colorEnd[1], colorEnd[2]);
      context.uniform3f(colorOverflowUniformLocation,  colorOverflow[0], colorOverflow[1], colorOverflow[2]);
        
      context.uniform1f(fractalRLocation,  fractalR);
      context.uniform2f(fractalCLocation, iterationFractalC[0], iterationFractalC[1]);
